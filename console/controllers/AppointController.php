    ...
    public function actionState(){
        ...
        foreach ($appoints as $k=>$v){
            ...
            if(!in_array($v->vaccine,[64,66,46])) {
                $app = Appoint::find()->where(['state' => [1,0]])->andWhere(['type' => $v->type])->andWhere(['phone' => $v->phone])->one();
                if ($app) {
                    $v->state = 3;
                    $v->cancel_type = 5;
                    $v->save();
            ...
            if($v->doctorid==38 && in_array($v->vaccine,[142,140,139,138,137,136,135,133,80,74,73,70,69,66,65,64,63,46])){
                $v->state = 0;
            }else {
                $v->state = 1;
            }
