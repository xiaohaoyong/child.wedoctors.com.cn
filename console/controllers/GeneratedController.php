...
class GeneratedController extends Controller
{
    public function actionQuestion1(){
        ...
        $quaa=QuestionNaireAnswer::find()->select('userid,createtime,doctorid')->groupBy('userid,createtime')->orderBy('createtime desc')->all();
        foreach($quaa as $k=>$v){
            ...
            system("ossutil64 cp /tmp/Question1.pdf oss://webdoctorsquestion/".$v->doctorid."/".date('Ymd')."/".$v->userid.".pdf -f");
        }
