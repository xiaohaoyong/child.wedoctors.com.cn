    ...
    <section class="talk_doctor">
      ...
      <div class="input_box" ref="input_box">
        <group class="input_area">
          <x-textarea maxlength="150" @on-change="onInput" @on-focus="onInput" id="input_speak" v-model.trim="speak" :max="200" :rows="1" placeholder="输入咨询问题" :show-counter="false" autosize></x-textarea>
        </group>
        <div class="menus">
...
//          self.qa_list.push({'type': 'talk','qa': 'a','text': '用户信息获取错误，请联系管理员'});
      ...
      scrollBottom() {
        ...
        if (parseFloat(version)>=11) {
          self.$nextTick(() => {
            self.$refs.talk_scroller.scrollTop = -(400+'px');
            /*setTimeout(()=>{
              document.body.scrollTop = document.documentElement.scrollTop = (document.documentElement.offsetHeight/2 + self.$refs.input_box.getBoundingClientRect().height);
            }, 400)*/
          });
        }else{
          self.$nextTick(() => {
      ...
      documentscrollBottom() {//ios
        ...
        if (parseFloat(version)>=11) {
          self.$nextTick(() => {
            /*setTimeout(()=>{
              document.body.scrollTop = document.documentElement.scrollTop = (500+self.$refs.talk_scroller.scrollHeight+document.documentElement.offsetHeight + self.$refs.input_box.getBoundingClientRect().height);
            })*/
          });
        }else{
          self.$nextTick(() => {
