<template>
  ...
  <section class="talk_parent">
    <my-title>{{doctorName}}</my-title>
    <div class="talk_box" ref="talk_scroller">
      <ul class="talk_list">
        <!--<li class="time">
          <time>2017-10-18  下午13:51</time>
        </li>
        <li class="talk a">
          <span class="avatar"><img src="./avatar.png" alt=""></span>
          <div class="text"><p>请问患者是否晕眩？<i class="angle"></i></p></div>
        </li>
        <li class="time">
          <time>2017-10-18  下午13:51</time>
        </li>
        ...
        <li v-for="item in qa_list"
          ...
          <span v-if="item.type == 'talk'" class="avatar">
            <img src="./avatar.png" alt="" v-if="item.qa == 'q'">
    ...
    <!--<vue-uploader server="serverPath"></vue-uploader>-->
...
<script type="text/ecmascript-6">
  ...
  import { Cell, Group,XTextarea,XButton,Badge } from 'vux'
  import {getVersion} from  '../../common/js/vaildform'
  import {httpTalkParent,httpTalkKeep} from 'my_api/talk'
  /*import Vue from "vue"
  ...
  export default {
    ...
    created() {
      this.userid = 0;
      this.doctorName = this.$route.query.doctor + '医生';
    ...
    beforeRouteLeave (to, from, next) {
      clearTimeout(this.timer);//不在当前页面--销毁清除定时器 避免重复请求
      next();
    },
    mounted() {
      let self = this;
...
//          self.qa_list.push({'type': 'talk','qa': 'a','text': '用户信息获取错误，请联系管理员'});
    ...
    components: {
      myTitle,Cell, Group,XTextarea,XButton,Badge
    }
  }
</script>
