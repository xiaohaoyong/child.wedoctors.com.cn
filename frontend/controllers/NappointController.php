...
class NappointController extends Controller
{
    ...
    public function actionIndex($search = '', $county = 0)
    {
        ...
        foreach ($doctors as $k => $v) {
            ...
            $hospitalAppoint = HospitalAppoint::findOne(['doctorid' => $v->userid, 'type' => 12]);
            if ($hospitalAppoint) {
    ...
    public function actionFrom($userid, $vid = 0)
    {
        ...
        $hospitalA = HospitalAppoint::findOne(['doctorid' => $userid, 'type' => 12]);

        if ($post = \Yii::$app->request->post()) {

            $appoint_log = Appoint::find()->where(['userid' => $this->login->userid, 'type' => 12])->andWhere(['<', 'state', 3])->one();
            if ($appoint_log) {
                ...
                if ($appointAdult->load($post) && $appointAdult->validate()) {
                    if ($appointAdult->save()) {
                        ...
                        if ($appoint->load($post) && $appoint->validate()) {
                            ...
                            if ($appoint->save()) {
                                return $this->redirect(['nappoint/view', 'id' => $appoint->id]);
                            }else{
                                var_dump($appoint->firstErrors);exit;
                            }
                        }
    ...
    public function actionDayNum($doctorid, $day)
    {
        ...
        $rs = [];
        $times = [];
        $hospitalA = HospitalAppoint::findOne(['doctorid' => $doctorid, 'type' => 12]);
    ...
    {
        $appoints = Appoint::findAll(['userid' => $this->login->userid, 'type' => 12, 'state' => $type]);
        $list = [];
    ...
    public function actionState($id, $type)
    {
        ...
        if (!$model) {
            ...
            return $this->redirect(['nappoint/my']);
        } else {
