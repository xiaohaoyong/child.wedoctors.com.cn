...
class CorpAPI extends API
{
     ...
     * @return
     */
    public function ExtUserList($userid)
    {
        self::_HttpCall(self::EXT_USER_GET_LIST, 'GET', array('userid'=>$userid));
        return $this->rspJson;
    }

    /**
     * @brief获取客户数据详情
     * @param $ext_userid  ：客户外部id
     * @return
     */
    public function ExtUserDetail($ext_userid)
    {
        self::_HttpCall(self::EXT_USER_GET_DETAIL, 'GET', array('external_userid'=>$ext_userid));
        return $this->rspJson;
    }
     ...
     * @param $userid  ：客服id
     * @param $ext_userid ： 客户外部id
     * @param $add_tag ： 需要添加的标签id数组
     * @param $remove_tag ：需要移除的标签id数组
     * @return
     */
    public function ExtUserTagSave($userid,$ext_userid,$add_tag,$remove_tag)
    {
        self::_HttpCall(self::GORP_TAG_SAVE, 'POST', [
            'userid'=>$userid,
     ...
     * @return
     */
    public function GorpTagGetList()
    {
        self::_HttpCall(self::GORP_TAG_GET_LIST, 'GET', array());
        return $this->rspJson;
    }
