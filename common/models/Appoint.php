...
//            $list = $queue->lrange();
    ...
    public function beforeSave($insert)
    {
        if ($insert) {
            ...
            if($this->source=='xiaoxiong'){
                Appoint::pushState($this->id);
            }
        }
        return parent::beforeSave($insert); // TODO: Change the autogenerated stub
    }
    public static function pushState($id){
        $appoint = Appoint::findOne($id);
        if($appoint->source == 'xiaoxiong'){
            $userTo=UserTo::findOne(['userid'=>$appoint->userid]);
            $appointSku = AppointSku::findOne(['aid'=>$id]);
            //获取用户微信登陆信息
            $path = "https://www.xiaoxiongjiankang.cn/order-notify/1";
            $curl = new HttpRequest( $path, true, 10);
            $curl->setData([
                'state'=>$appoint->state,
                'cancel_type'=>self::$cancel_typeText[$appoint->cancel_type],
                'skuid' =>$appointSku->skuid,
                'xuserid'=>$userTo->touserid,
