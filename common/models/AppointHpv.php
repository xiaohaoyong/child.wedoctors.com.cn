...
class AppointHpv extends \yii\db\ActiveRecord
{
    ...
    public function beforeSave($insert)
    {
        if($insert){
            ...
            if($this->date<'1990-01-01' && $this->state==1){
                $Ah=AppointHpv::find()->where(['doctorid'=>Yii::$app->user->identity->doctorid])
                    ->andWhere(['state'=>1])
                    ->orderBy('date desc')
                    ->one();
                if($Ah->date>'0000-01-01'){
                    $date='';
                    $daynum=AppointHpv::find()->where(['doctorid'=>Yii::$app->user->identity->doctorid])
                        ->andWhere(['date'=>$Ah->date])
                        ->andWhere(['state'=>1])
                        ->count();
                    $week = date('w', strtotime($Ah->date));
                    $aw=AppointHpvSetting::find()->where(['doctorid'=>Yii::$app->user->identity->doctorid])->one();
                    $key='week'.$week;



                    while ($date){
                        $d=strtotime(strtotime($date),'+1 day');
                    }
                }
            }
        }
